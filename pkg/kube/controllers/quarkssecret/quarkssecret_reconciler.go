...
func (r *ReconcileQuarksSecret) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	//		Name: qsec.Spec.Request.CertificateRequest.CARef.Name,
	//	}, caSecret)
	//	if apierrors.IsNotFound(err) {
	//		return reconcile.Result{RequeueAfter: time.Second * 5}, nil
	//	} // else maybe we handle other errors
	//
	//	// this will also have to skip 1) isCA: true
	//	//err = r.createCertificateSecret(ctx, qsec)
	//	//convertToTLSType
	//
	...
	case qsv1a1.LocalSigner:
		...
		if qsec.Spec.Type == "tls" {
			secret = &corev1.Secret{
				...
				Type: "kubernetes.io/tls", // TODO is there a constant we can use?
			}
		} else {
...
// Skip creation when
// * secret is already generated by Quarks before according to qsecs status field
// * secret exists, but was not generated by Quarks (but instead user created secret)


//Skip creation when secret exists
func (r *ReconcileQuarksSecret) skipCreation(ctx context.Context, qsec *qsv1a1.QuarksSecret) (bool, error) {
	// if the QuarksSecret object (qsec1)'s status labelled as generated:true (aka the desired Secret specified by qsec1 has been generated by Quarks already)
	...
	// the desired Secret specified by qsec1 has not been generated by Quarks yet
	...
	// the desired Secret specified by qsec1 has not been generated by Quarks yet
	// & there is already a Secret in k8s called `foo`

	// what is possible here:
	// 1) `foo` was generated by qsec0 yesterday
	// 2) `foo` was generated manually/other software
	secretLabels := existingSecret.GetLabels()
	if secretLabels == nil {
		secretLabels = map[string]string{}
	}

	// skip if the secret was not created by the QuarksSecret
	// = skip if the secret was created manually (case 1)
	if secretLabels[qsv1a1.LabelKind] != qsv1a1.GeneratedSecretKind {
		// if the secretLabels["quarks.cloudfoundry.org/secret-kind"] != "generated"
		// eg. the labels dont have "quarks.cloudfoundry.org/secret-kind" key
	...
	// if `foo` was generated by qsec0 yesterday
	return false, nil
}
